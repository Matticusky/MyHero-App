...
File transfer uses raw binary data for efficient transmission over BLE. The device negotiates a higher MTU (256 bytes) for optimal throughput.
...
| Chunk Size | Max 244 bytes (raw binary) |
...
- Max 244 bytes per chunk
- With MTU 256, max ATT payload is 253 bytes; 244 leaves margin for protocol overhead
...
- Max chunk size: 244 bytes
- MTU negotiated to 256 bytes for optimal throughput
...
6. UPLOAD FILE
   ...
   - Send raw binary chunks (max 244 bytes each)
...
async function uploadFile(filename, fileData) {
    ...
    // Send raw binary chunks (max 244 bytes each)
    for (let i = 0; i < fileData.length; i += 244) {
        const chunk = fileData.slice(i, i + 244);
...
|---------|------|---------|
| 1.2 | 2026-02-04 | Removed Base64 encoding for faster transfer. Now uses raw binary (244 bytes/chunk). Optimized connection parameters (7.5-15ms interval). |
