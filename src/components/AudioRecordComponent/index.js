...
const AudioRecordComponent = ({
  ...
  const checkPermissions = async () => {
    if (Platform.OS === 'android') {
      const recordAudioPermission = await request(
  ...
  const onStartRecord = async () => {
    if (!hasPermission) {
      return Alert.alert(
        'Error',
        'Please grant the necessary permissions to record audio',
      );
    }
  ...
  const onStopRecord = async () => {
    try {
      ...
      onSave(result); // Pass the file path to the parent
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      <Pressable style={styles.recordButton} onPress={onStartRecord}>
        {recording && !paused ? <Icons.PauseIcon /> : <Icons.MicIcon />}
      </Pressable>
      <View style={styles.bottomBar}>
        {audioFilePath && <Text style={styles.recordTime}>{recordTime}</Text>}
...
const styles = StyleSheet.create({
  container: {
    ...
    height:
      Platform.OS === 'android' ? UtilityMethods.hp(10) : UtilityMethods.hp(6),
    position: 'relative',
  },
  recordButton: {
    backgroundColor: Colors.BLACK,
    ...
    position: 'absolute',
    top:
      Platform.OS === 'android' ? -UtilityMethods.hp(6) : -UtilityMethods.hp(6),
